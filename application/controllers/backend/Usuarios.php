...
class Usuarios extends CI_Controller
{
    ...
    public function store()
    {
        ...
        $this->form_validation->set_rules('ci', 'ci', 'trim|required|exact_length[8]|is_unique[usuarios.ci]', array('required' => 'Debes proporcionar un %s.', 'is_unique' => 'Este %s ya existe', 'exact_length' => 'El %s debe ser 8 digitos'));
    ...
    public function update()
    {
        ...
        if ($ci != $usuario_actual->ci) {
            $is_unique_ci = '|is_unique[usuarios.ci]';
        } else {
            $is_unique_ci = '';
        }
        ...
        $this->form_validation->set_rules('nombres', 'Nombres', 'trim|required', array('required' => 'Debes proporcionar un %s.'));
        $this->form_validation->set_rules('apellidos', 'Apellidos', 'trim|required', array('required' => 'Debes proporcionar un %s.'));
        $this->form_validation->set_rules('ci', 'ci', 'trim|required|exact_length[8]' . $is_unique_ci, array('required' => 'Debes proporcionar un %s.', 'is_unique' => 'Este %s ya existe', 'exact_length' => 'El %s debe ser 8 digitos'));
