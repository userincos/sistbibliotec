...
class Lectores extends CI_Controller
{
    ...
    public function store()
    {
        ...
        $this->form_validation->set_rules('ci', 'ci', 'trim|required|exact_length[8]|is_unique[lectores.ci]', array('required' => 'Debes proporcionar un %s.', 'is_unique' => 'Este %s ya existe', 'exact_length' => 'El %s debe ser 8 digitos'));
    ...
    public function update()
    {
        ...
        if ($ci != $lector_actual->ci) {
            $is_unique = '|is_unique[lectores.ci]';
        } else {
            $is_unique = '';
        }
        $this->form_validation->set_rules('nombres', 'Nombres', 'trim|required', array('required' => 'Debes proporcionar un %s.'));
        $this->form_validation->set_rules('apellidos', 'Apellidos', 'trim|required', array('required' => 'Debes proporcionar un %s.'));
        $this->form_validation->set_rules('ci', 'ci', 'trim|required|exact_length[8]' . $is_unique, array('required' => 'Debes proporcionar un %s.', 'is_unique' => 'Este %s ya existe', 'exact_length' => 'El %s debe ser 8 digitos'));
        $this->form_validation->set_rules('direccion', 'Direccion', 'trim|required', array('required' => 'Debes proporcionar un %s.'));
        $this->form_validation->set_rules('idtipolector', 'Tipo Lector', 'trim|required', array('required' => 'Debes proporcionar un %s.'));
        $this->form_validation->set_error_delimiters('<span class="help-block">', '</span>');
    ...
    public function comprobarci()
    {
        $ci = $this->input->post("ci");
        $res = $this->Lectores_model->comprobarci($ci);
        if ($res) {
            if ($res->estado == 1) {
